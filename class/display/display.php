<?php
    ...
    <ul class="dropdown-menu">
        ...
        <li><a href="javascript:;" class="delete-room" data-id="'.$value["id"].'"><i class="fa fa-trash"></i> Delete</a></li>
				...
				foreach ($fetch as $key => $value) {
					...
					$button = 	'<div class="btn-group">
									...
									<ul class="dropdown-menu">
										...
										<li><a href="javascript:;" class="delete-member" data-id="'.$value["id"].'"><i class="fa fa-trash"></i> Delete</a></li>
									</ul>
								</div>';
				...
				foreach ($fetch as $key => $value) {
					...
					$button = 	'<div class="btn-group">
									...
									<ul class="dropdown-menu">
										...
										<li><a href="javascript:;" class="delete-user" data-id="'.$value["id"].'"><i class="fa fa-trash"></i> Delete</a></li>
									</ul>
								</div>';
	...
	public function display_equipment_lost()
{
	...
	$sql = $conn->prepare('SELECT item.i_model, item.i_category, item.i_brand, 
		SUM(item_inventory.inventory_itemstock) as inv_stock, 
		item_inventory.item_remarks 
		FROM item_inventory
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
	if (isset($_POST['key']) && $_POST['key'] === 'display_equipment_pulled') {
		$display = new Display();
